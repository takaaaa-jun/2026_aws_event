...
from models import DoctorlessCityInternal
from path_info import get_doctorless_area_path
from log_decorator import Logger
from session import session

DOCTORLESS_CITY_INTERNAL_CSV = get_doctorless_area_path() / "doctorless_city_internal.csv"
...
def import_csv_data():
    ...
    print("既存の無医地区（内科用市区町村）データをクリアしています...")
    session.query(DoctorlessCityInternal).delete()
    session.commit()

    if DOCTORLESS_CITY_INTERNAL_CSV.exists():
        print(f"無医地区（内科用市区町村）データのインポートを開始: {DOCTORLESS_CITY_INTERNAL_CSV.name}")
        with open(DOCTORLESS_CITY_INTERNAL_CSV, mode="r", encoding="utf-8-sig") as f:
            reader = csv.DictReader(f)
            for row in reader:
                doctorless_city = DoctorlessCityInternal(
        ...
        print(f"警告: CSVファイルが見つかりません: {DOCTORLESS_CITY_INTERNAL_CSV}")
